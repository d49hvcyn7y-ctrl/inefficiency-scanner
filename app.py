...
TOP_STOCKS = [
    ...
    "XOM","CVX","CAT","GE","HON","DIS","NFLX","NEE","SO","BA"
]

def scan_stocks(max_tickers=40):
    if not YF_AVAILABLE:
        return pd.DataFrame()
    ...
    return df.sort_values("Stretch Score", ascending=False).reset_index(drop=True) if not df.empty else df
...
stock_limit = st.slider("How many stocks to scan", 20, 40, 30)
...
if st.button("🔄 Scan for Inefficiencies", type="primary"):
    if mode == "Crypto Cross-Exchange Spreads":
        with st.spinner("Checking live crypto prices..."):
            df, details = scan_crypto(threshold)
            if df.empty:
                st.warning("Could not get prices right now. Try again.")
            else:
                st.dataframe(df, use_container_width=True)
                good = df[df["Flag"] == "✅"]
                if not good.empty:
                    st.success(f"Found {len(good)} interesting spreads")
                else:
                    st.info("No big spreads at the moment")
    
    elif mode == "Top Stocks Technical Screen":
        with st.spinner("Scanning stocks (may take 20-40 seconds)..."):
            df = scan_stocks(stock_limit)
            if df.empty:
                st.warning("Stock data is busy right now. Try again in a minute or lower the number of stocks.")
            else:
                st.success(f"Scanned {len(df)} stocks")
                st.dataframe(df, use_container_width=True)
                oversold = df[df["% vs 200DMA"] < -12]
                if not oversold.empty:
                    st.subheader("Possible mean-reversion candidates")
                    st.dataframe(oversold, use_container_width=True)
    
    elif mode == "Relative Value Calculator":
        st.info("Use the inputs below and click Calculate")
